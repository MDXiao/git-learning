...
class Lab1Spec(lab1: Lab1Like) extends FlatSpec {
  ...
  val myTree = Node(Node(Node(Empty,3,Empty),4,Empty), 7, Node(Node(Empty,8,Empty),11,Node(Empty,13,Empty)))


  //magicFunc

  "magicFunc" should "add all the even number in the tree up" in {
    assert(magicFunc(myTree) === 12)
  }

...
import jsy.lab4.Parser.parse //For the parser
...
class Lab4Spec(lab4: Lab4Like) extends FlatSpec {
...

  {
    ...
    val testFunc = parse("function(x: number) { return x } (4)")

    "step" should "evaluate function ID test" in {
      val result = iterateStep(testFunc)
      assert(result == N(4.0))
    }

    "Eval/Eq" should "return true if two numerical values are the same" in {
      val e1 = N(5)
      val e2 = N(5)
      val e3 = step(Binary(Eq, e1, e2))
      assert(e3 === B(true))
    }

...
class Lab3Spec(lab3: Lab3Like) extends FlatSpec {
  ...
  }
  {
    ...
    val test1 = parse("const x = 1; const g = (y) => x; const h = (x) => g(2); h(3)")
    val test2 = parse("const x = 3; const g = function (y) { return x + x }; const h = function (x){ return g(y)}; h(2)")
    val test3 = parse("const x = 3; const g = function (y){ return x + x }; const h = function (x){ return g(y)}; console.log(h(2))")

    "eval" should "perform dynamic scoping correctly on test1" in {
      assert(evaluate(test2) == N(3))
    }

    "step" should "perform static scoping correctly on test1" in {
      val result = iterateStep(test2)
      assert(result == N(1))
    }
